...
printHelp() {
    ...
    echo "-s : don't bypass fabric-samples repo clone"
    echo "-b : bypass download of platform-specific binaries"
    echo
    echo "e.g. bootstrap.sh 1.4.1 -s"
    echo "would download docker images and binaries for version 1.4.1"
    echo 
    echo "If no paramters will be passed only the system specific binaries will be downloaded"
}
...
binaryIncrementalDownload() {
    ...
    if [ -z "$rc" ] || [ $rc -eq 33 ] || [ $rc -eq 2 ]; then
        ...
        if [ "$localMd5sum" == "$remoteMd5sum" ]; then
            ...
            tar xzf ./"${BINARY_FILE}" bin --overwrite
            echo "==> Done."
