...
function printHelp() {
  ...
  echo
  echo "	build-network.sh generate -c mychannel"
  echo "	build-network.sh up -c mychannel -s couchdb"
  echo "        build-network.sh up -c mychannel -s couchdb -i 1.4.0"
  echo "	build-network.sh up -l node"
  echo "	build-network.sh down -c mychannel"
  echo "        build-network.sh upgrade -c mychannel"
  echo
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $BLACKLISTED_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      echo "ERROR! Local Fabric binary version of $LOCAL_VERSION does not match this newer version of the script and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi

    echo "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      echo "ERROR! Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match this newer version of the script and is unsupported. Either move to a later version of Fabric or checkout an earlier version of fabric-samples."
      exit 1
    fi
  done
}
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.emsa.europa.eu | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.emsa.europa.eu
    docker cp -a orderer.emsa.europa.eu:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.emsa.europa.eu
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.emsa.europa.eu

    for PEER in peer0.dma.dk peer1.dma.dk peer0.veeteedeamet.ee peer1.veeteedeamet.ee; do
    ...
    docker exec cli scripts/upgrade_to_v14.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE $COLLECTIONS_PATH
    if [ $? -ne 0 ]; then
      echo "ERROR !!!! Test failed"
      exit 1
    fi
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/dma.dk/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/veeteedeamet.ee/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# (``peer0.dma.dk`` & ``peer0.veeteedeamet.ee``).  Secondly, we point to
